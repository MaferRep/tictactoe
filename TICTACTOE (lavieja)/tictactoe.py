...
def checkWinner(jugador):
    global background, won, startX, startY, endX, endY

    for i in range(8):
        if board[winners[i][0]] == jugador and board[winners[i][1]] == jugador and board[winners[i][2]] == jugador:
            won = True
    ...
    if won:
        ...
        casilla_group.empty()
        background = p.image.load(jugador.upper() + ' ganador.png')
...
def Winner(jugador):
    ...
    for i in range(8):
        if board[winners[i][0]] == jugador and board[winners[i][1]] == jugador and board[winners[i][2]] == '':
            compMove = winners[i][2]
            move = False

        elif board[winners[i][0]] == jugador and board[winners[i][1]] == '' and board[winners[i][2]] == jugador:
            compMove = winners[i][1]
            move = False

        elif board[winners[i][0]] == '' and board[winners[i][1]] == jugador and board[winners[i][2]] == jugador:
...
def CompMove():
    ...
    if not move:
        for casilla in casillas:
            if casilla.number == compMove:
                casilla.clicked(casilla.x, casilla.y)

    else:
        Update()
        time.sleep(0.5)
        casilla_group.empty()
        background = p.image.load('empate.png')
        background = p.transform.scale(background, (WIDTH, HEIGHT))
...
def Update():
    ...
    casilla_group.draw(win)
    casilla_group.update()
    p.display.update()
